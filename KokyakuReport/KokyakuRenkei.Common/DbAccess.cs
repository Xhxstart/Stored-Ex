    ...
    public class DbAccess
    {
        ...
        public int ExecuteNonQuery(string query, object[] parameters)
        {
            ...
            try
            {
                ...
                SetParameter(parameters);
                logger.Debug(Utility.GetMsg(MsgConst.KK0009I) + cmd.CommandText);
                OutputParameters(cmd.Parameters);
                count = cmd.ExecuteNonQuery();
            }
            catch (Exception ex)
            {
                logger.Error(Utility.GetMsg(MsgConst.KK0007E));
                throw;
            }
            return count;
        }
        ...
        private DbType GetDbType(object arg)
        {
            ...
            else throw new ArgumentException(Utility.GetMsg(MsgConst.KK0005E));
        }
        ...
        private void OutputParameters(SqlParameterCollection parameters)
        {
            if (parameters != null)
            {
                ...
                {
                    logger.Debug(Utility.ReleaseMsg(Utility.GetMsg(MsgConst.KK0010I), new string[] { (x.idx + 1).ToString(), x.parameter.ParameterName, x.parameter.SqlValue.ToString() }));
                });
            }
        }
